...
if(isset($_REQUEST["html_ok"])) //checkbox는 체크해야 변수명 전달됨.
 ...
 $sql = "update test.greet set subject=?, content=?, is_html=? where num=?";
...
$pdo->commit();
 ...
 header("Location:http://10.211.55.3/greet/list.php");
...
$is_html = "";
 $content = htmlspecialchars($content);
    ...
    try{
 $pdo->beginTransaction();
 $sql = "insert into test.greet(id,name,nick,subject,content,regist_day,hit,is_html)";
 $sql .= "values(?, ?, ?, ?, ?, now(), 0, ?)";
 $stmh = $pdo->prepare($sql);
