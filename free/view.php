...
$file_dir = '/usr/local/apache2.4/htdocs/data/';
$num=$_REQUEST["num"];
 require_once("../lib/MYDB.php");
 $pdo = db_connect();

 try{
 $sql = "select * from test.free where num=?";
...
$item_content = str_replace("\n", "<br>", $item_content);
 ...
 $sql = "update test.free set hit=? where num=?"; // 글 조회수 증가
$stmh = $pdo->prepare($sql);
...
<div id="view_title1"><?= $item_subject ?></div>
 <div id="view_title2"><?= $item_nick ?> | 조회 : <?= $item_hit+1 ?> | <?= $item_date ?>
</div>
 ...
 <div id="view_content">
                ...
                {
                if ($image_copied[$i]) {
                $imageinfo = getimagesize($file_dir.$image_copied[$i]);$image_width[$i] = $imageinfo[0];
...
print "<img src='$ 11img_name' width='$image_width[$i]'><br><br>";
 }
 }
 }
 ?>
...
if($_SESSION["userid"]=="admin" || $_SESSION["userid"]==$ripple_id)
 ...
 ?>
         <form name="ripple_form" method="post"
             ...
             <?php
             if($_SESSION["userid"]=="yjh6487" || $_SESSION["userid"]=="yjh1111" || $_SESSION["userid"]=="admin" || $_SESSION["level"]==1 ){
             ?>
             
              <div id="ripple_box">
 <div id="ripple_box1"><img src="../img/title_comment.gif"></div>
 <div id="ripple_box2"><textarea rows="5" cols="65" name="ripple_content"
                                                     required></textarea>
 </div>
