...
$files = $_FILES["upfile"]; //첨부파일
 ...
 $upload_dir = '/usr/local/apache2.4/htdocs/data\\'; //물리적 저장위치
for ($i=0; $i<$count; $i++)
 {
...
$del_ok[$index] = "y";
     ...
     try{
 $sql = "select * from test.free where num=?"; // get target record$stmh = $pdo->prepare($sql);
     ...
     for ($i=0; $i<$count; $i++) {
         ...
         if ($del_ok[$i] == "y")
         {
             ...
             try {
                 ...
                 $sql = "update test.free set $field_org_name = ?, $field_real_name = ? where num=?";
                 $stmh = $pdo->prepare($sql);
 ...
 try{
 $pdo->beginTransaction();
 $sql = "update test.free set subject=?, content=?, is_html=? where num=?";
 $stmh = $pdo->prepare($sql);
...
$is_html = "";
 $content = htmlspecialchars($content);
    ...
    try{
 $pdo->beginTransaction();
 $sql = "insert into test.free(id,name,nick,subject,content,regist_day,hit,is_html, ";
...
$stmh->bindValue(9, $upfile_name[2], PDO::PARAM_STR); $stmh->bindValue(10, $copied_file_name[0], PDO::PARAM_STR);
...
header("Location:http://10.211.55.3/free/list.php");
 ?>